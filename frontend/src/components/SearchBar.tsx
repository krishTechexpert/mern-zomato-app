...
export default function SearchBar({onSubmit,placeholder,onReset,searchQuery}:Props) {
  const form = useForm<SearchForm>({
    resolver: zodResolver(formSchema), // it initilize default values 
    defaultValues:{
      searchQuery
    }
  })
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className={`flex items-center  gap-3 justify-between flex-row border-2 rounded-full p-3 mx-5 ${form.formState.errors.searchQuery && "border-red-500"}`}>
        ...
        <FormField control={form.control} name="searchQuery"  render={({field}) => (
           ...
           {/* isDirty = input touched and have value */}
